  ...
  if (!user || loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        ...
        <p className="text-foreground/40 font-black text-[10px] uppercase tracking-widest">Chargement de votre univers...</p>
      </div>
    );
  }
    ...
    <div className="container mx-auto py-12 px-4 max-w-7xl">
      <div className="flex items-center justify-between mb-12">
        <div className="space-y-1">
          <h1 className="text-4xl font-black text-foreground tracking-tighter italic">Espace Personnel</h1>
          <p className="text-[10px] font-bold text-foreground/40 uppercase tracking-[0.2em]">Rédigez votre histoire, gérez vos trésors</p>
        </div>
        {user.role === 'SUPPLIER' && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 items-start">
        {/* Sidebar */}
        <aside className="lg:col-span-1 bg-white rounded-[40px] p-6 shadow-xl shadow-foreground/5 border border-border/30">
          <div className="flex flex-col items-center text-center gap-4 mb-10 px-2 pt-4">
            <div className="relative group">
               <div className="w-24 h-24 bg-primary rounded-[35px] flex items-center justify-center text-white font-black text-3xl shadow-2xl shadow-primary/20 overflow-hidden">
                 {formData.avatarUrl ? (
               ...
               <button 
                ...
                className="absolute -bottom-2 -right-2 w-10 h-10 bg-foreground text-white rounded-2xl flex items-center justify-center border-4 border-white shadow-lg  hover:bg-primary transition-all"
               >
                  <Camera size={16} />
               </button>
            </div>
            <div className="space-y-1">
               <p className="text-base font-black text-foreground tracking-tight truncate max-w-[200px]">{user.role === 'SUPPLIER' ? formData.shopName : formData.name}</p>
               <div className="flex items-center justify-center gap-2">
                 <div className={`w-1.5 h-1.5 rounded-full ${user.role === 'SUPPLIER' ? 'bg-primary' : 'bg-leaf'}`} />
                 <span className="text-[10px] font-black text-foreground/40 uppercase tracking-widest">{user.role}</span>
               </div>
            </div>
          </div>
          ...
          <div className="mt-10 pt-6 border-t border-border/30">
             <button 
               ...
               className="w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest text-primary/60 hover:bg-primary/5 transition-all"
             >
                <LogOut size={18} />
                Déconnexion
             </button>
        ...
        <main className="lg:col-span-3">
          <div className="bg-white rounded-[50px] p-12 shadow-2xl shadow-foreground/5 border border-border/30 min-h-[600px]">
             <form onSubmit={handleSave} className="space-y-12">
               
               {activeTab === 'PROFILE' && (
                 <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="space-y-2">
                        <h2 className="text-3xl font-black text-foreground italic tracking-tight">Mon Identité</h2>
                        <p className="text-[10px] font-bold text-foreground/40 uppercase tracking-[0.2em]">Gérez vos informations de contact</p>
                    </div>
               ...
               {activeTab === 'SECURITY' && (
                 <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-xl">
                    <div className="space-y-2">
                        <h2 className="text-3xl font-black text-foreground italic tracking-tight">Sécurité</h2>
                        <p className="text-[10px] font-bold text-foreground/40 uppercase tracking-[0.2em]">Protégez vos données artisanales</p>
                    </div>
               ...
               {activeTab === 'SHOP' && user.role === 'SUPPLIER' && (
                 <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="space-y-2">
                        <h2 className="text-3xl font-black text-foreground italic tracking-tight">Ma Boutique Teranga</h2>
                        <p className="text-[10px] font-bold text-foreground/40 uppercase tracking-[0.2em]">Gérez votre présence sur la marketplace</p>
                    </div>
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                         ...
                         <div className="space-y-4">
                            <label className="text-[10px] font-black uppercase tracking-widest text-foreground/40 px-2 block">Médias</label>
                            <div className="flex gap-4">
                               <div className="flex-1 p-6 bg-background border-2 border-dashed border-border rounded-3xl flex flex-col items-center gap-2 group cursor-pointer hover:border-primary/30">
                                  <ImageIcon size={20} className="text-foreground/20" />
                                  <span className="text-[9px] font-black uppercase tracking-widest text-foreground/30">Logo</span>
                               </div>
                               <div className="flex-1 p-6 bg-background border-2 border-dashed border-border rounded-3xl flex flex-col items-center gap-2 group cursor-pointer hover:border-primary/30">
                                  <ImageIcon size={20} className="text-foreground/20" />
               ...
               {activeTab === 'FAVORITES' && user.role === 'CLIENT' && (
                 <div className="space-y-8 animate-in fade-in duration-500">
                    <div className="space-y-2">
                        <h2 className="text-3xl font-black text-foreground italic tracking-tight">Mes Perles Favorites</h2>
                        <p className="text-[10px] font-bold text-foreground/40 uppercase tracking-[0.2em]">Vos articles préférés en un coup d'œil</p>
                    </div>
                    
                    {favorites.length === 0 ? (
                      <div className="py-20 text-center space-y-6">
                        <Heart size={64} className="mx-auto text-foreground/10 mb-4" />
               ...
               {activeTab === 'NOTIFICATIONS' && (
                  <div className="py-24 text-center space-y-6 animate-in zoom-in-95 duration-500">
                     <Bell size={64} className="mx-auto text-foreground/10" />
                     <h3 className="text-2xl font-black text-foreground italic">Alertes Teranga</h3>
                     <p className="text-[10px] font-bold text-foreground/30 uppercase tracking-[0.2em] max-w-sm mx-auto">Configurez vos préférences de réception dès que cette perle sera disponible.</p>
                  </div>
               )}
